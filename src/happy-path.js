...
export function showHappyPath(bpmnVisualization) {
  ...
  happyPath.forEach((elementId, index) => {
    ...
    if (isActivity(elementId) || isEvent(elementId)) {
      classesToAdd = [ "pulse-happy", `animate-${elementId}`];
    } else if (isGateway(elementId)) {
      classesToAdd = [ "gateway-happy", `animate-${elementId}` ];
    } else { //flow
      classesToAdd = [ "growing-happy", `animate-${elementId}` ];
    }

    bpmnVisualization.bpmnElementsRegistry.addCssClasses(elementId, classesToAdd);
  });
  /*  happyPath.forEach((elementId, index) => {
      ...
      let classesToAdd;
      if (isActivity(elementId) || isEvent(elementId)) {
        styleElt.innerHTML = styleElt.innerHTML + `\n.bpmn-type-activity.animate-${elementId} > ${isActivity(elementId) ?'rect': 'ellipse'} { animation-delay: ${(index) * 5}s; }`;
        classesToAdd = [ "highlight-happy-path", "pulse-happy", `animate-${elementId}`];
      } else if (isGateway(elementId)) {
        styleElt.innerHTML = styleElt.innerHTML + `\n.bpmn-type-activity.animate-${elementId} > 'path' { animation-delay: ${(index) * 5}s; }`;
        classesToAdd = [ "gateway-happy", `animate-${elementId}` ];
      } else { //flow
        styleElt.innerHTML = styleElt.innerHTML + `\n.bpmn-type-activity.animate-${elementId} > 'path' { animation-delay: ${(index) * 5}s; }`;
        classesToAdd = [ "growing-happy", `animate-${elementId}` ];
