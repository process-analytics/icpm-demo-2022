import { FlowKind, ShapeBpmnElementKind } from 'bpmn-visualization';
...
function createLinearGradient(
  ...
  const activitySvgElement = bpmnVisualization.bpmnElementsRegistry.getElementsByIds(elementId)[0]
    .htmlElement;
...
export function hideConformanceData(bpmnVisualization) {
  ...
  const bpmnTaskElements = bpmnVisualization.bpmnElementsRegistry.getElementsByKinds(
    ShapeBpmnElementKind.TASK,
  );
  bpmnTaskElements.forEach((elt) => {
    bpmnElementsRegistry.removeAllOverlays(elt.bpmnSemantic.id);
  });
  const bpmnSubprocessElements = bpmnVisualization.bpmnElementsRegistry.getElementsByKinds(
    ShapeBpmnElementKind.SUB_PROCESS,
  );
  bpmnSubprocessElements.forEach((elt) => {
    bpmnElementsRegistry.removeAllOverlays(elt.bpmnSemantic.id);
  });
  const bpmnEventElements = bpmnVisualization.bpmnElementsRegistry.getElementsByKinds(
  ...
  const bpmnFlowElements = bpmnVisualization.bpmnElementsRegistry.getElementsByKinds(
